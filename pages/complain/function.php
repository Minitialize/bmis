...
if(isset($_POST['btn_save_complain'])){
    $chkblot = mysqli_query($con,"SELECT concat( YEAR(CURRENT_DATE()),'-', MAX(tc.id)) FROM tbl_complain tc");
    $chkblot = mysqli_fetch_fields($chkblot);
    $date = date('Y-m-d');
    $year = date('Y');
    echo $chkblot;
    $txt_compainto = $_POST['txt_compainto'];
    $txt_purpose = $_POST['txt_purpose'];
    $complaindesc = $_POST['txt_complaindescription'];
    $complainresponse = $_POST['txt_complain_response'];
    $time = date("h:i:sa");
    $query = mysqli_query($con,"INSERT INTO tbl_complain (caseno,residentid,againstto,datesched,time,createdon,purpose,complain_description,response,status) 
    values ('.$chkblot.', '".$_SESSION['userid']."','$txt_compainto','$date','$time',CURRENT_DATE(),'$txt_purpose','$complaindesc','$complainresponse','New')") or die('Error: ' . mysqli_error($con));
...
if(isset($_POST['btn_approve_complains']))
{
    
    $txt_id = $_POST['hidden_id'];
    $approve_query = mysqli_query($con,"UPDATE tbl_complain set status= 'Acknowledge' where id = '".$txt_id."' ") or die('Error: ' . mysqli_error($con));
...
if(isset($_POST['btn_settle_complains']))
{
    
    $txt_id = $_POST['hidden_id'];
    $approve_query = mysqli_query($con,"UPDATE tbl_complain set status= 'Settle' where id = '".$txt_id."' ") or die('Error: ' . mysqli_error($con));
...
if(isset($_POST['btn_disapprove_complain']))
{
    
    $txt_id = $_POST['hidden_id'];
    $approve_query = mysqli_query($con,"UPDATE tbl_complain set status= 'Disapproved' where id = '".$txt_id."' ") or die('Error: ' . mysqli_error($con));
